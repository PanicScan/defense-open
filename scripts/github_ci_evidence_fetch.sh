...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/github_ci_evidence_fetch.sh [OWNER/REPO] [RUN_ID] [EVIDENCE_ROOT]

Downloads the three CI evidence artifacts from a GitHub Actions CI run and then
runs scripts/ci_artifact_evidence_audit.sh against the extracted evidence root.

Environment overrides:
  PANICSCAN_GITHUB_REPO       OWNER/REPO; defaults to Cargo.toml repository URL
  PANICSCAN_GITHUB_RUN_ID     GitHub Actions run id; defaults to latest successful CI run
  PANICSCAN_CI_EVIDENCE_ROOT  download root; defaults to /tmp/panicscan-ci-evidence-<run-id>
USAGE
}
...
run_id="${2:-${PANICSCAN_GITHUB_RUN_ID:-}}"
evidence_root="${3:-${PANICSCAN_CI_EVIDENCE_ROOT:-}}"

if ! command -v gh >/dev/null 2>&1; then
  echo "gh CLI is required to download GitHub Actions evidence artifacts" >&2
  exit 1
fi
...
if [[ -z "$run_id" ]]; then
  echo "no successful CI workflow run found for $repo" >&2
  echo "Trigger CI first or pass PANICSCAN_GITHUB_RUN_ID." >&2
  exit 1
fi

if [[ -z "$evidence_root" ]]; then
  evidence_root="/tmp/panicscan-ci-evidence-$run_id"
fi
...
if find "$evidence_root" -mindepth 1 -maxdepth 1 -print -quit | grep -q .; then
  echo "evidence root is not empty: $evidence_root" >&2
...
scripts/ci_artifact_evidence_audit.sh "$evidence_root"
